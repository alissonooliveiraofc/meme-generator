...
const isOnSameHorizontal = (firstElement, secondElement) =>
  ...
  (secondElement.top >= firstElement.top &&
    secondElement.bottom <= firstElement.bottom);
...
describe('1 - Crie um input de texto', () => {
  beforeEach(() => {
...
describe('2 - Crie uma função que execute upload de imagem', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
  });
...
describe('3 - Adicione uma moldura no preview de visualização', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
    cy.reload();
...
describe('4 - Adicione o texto que será inserido sobre a imagem', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
    cy.reload();
  ...
  it('Será validado se o texto do elemento `meme-text` deve estar na cor branca', () => {
    cy.get('#meme-text').should('have.css', 'color', 'rgb(255, 255, 255)');
  });
});

describe('5 - Limite o tamanho do texto do meme', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
    cy.reload();
...
describe('6 - Crie três botões que alterem as cores da borda dos memes', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
  });
  ...
  it('Será validado se o botão com identificado com id `fire` funciona corretamente.', () => {
    cy.get('#fire').should('have.css', 'background-color', 'rgb(255, 0, 0)');
    cy.get('#fire').click();
    cy.get('#meme-image-container').should(
      'have.css',
      'border',
  ...
  it('Será validado se o botão com identificado com id `water` funciona corretamente.', () => {
    cy.get('#water').should('have.css', 'background-color', 'rgb(0, 0, 255)');
    cy.get('#water').click();
    cy.get('#meme-image-container').should(
      'have.css',
      'border',
  ...
  it('Será validado se o botão com identificado com id `earth` funciona corretamente.', () => {
    cy.get('#earth').should('have.css', 'background-color', 'rgb(0, 128, 0)');
    cy.get('#earth').click();
    cy.get('#meme-image-container').should(
      'have.css',
      'border',
...
describe('7 - Crie um conjunto de quatro imagens pré prontas que permita que a pessoa usuária possa escolher', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('/');
  });
